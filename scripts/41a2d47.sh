...
ensure_gh_environment() {
    ...
    echo "$(jq -n '{"deployment_branch_policy": {"protected_branches": false,"custom_branch_policies": true}}' | gh api -H "Accept: application/vnd.github+json" -X PUT "/repos/bswck/mempip/environments/$1" --input -)" > /dev/null 2>&1 || return 1
}
