...
#
# Interact with bswck/skeleton (current version: https://github.com/bswck/skeleton/tree/0.0.2rc-210-g3b97536).
#
# This file was generated from bswck/skeleton@0.0.2rc-210-g3b97536.
# Instead of changing this particular file, you might want to alter the template:
# https://github.com/bswck/skeleton/tree/0.0.2rc-210-g3b97536/project/scripts/skeleton.%7B%7Bsref%7D%7D.bash.jinja
#
# Usage:
#
...
make_context() {
	...
	COVERAGE_URL=$(cat <<- 'EOF'
		https://coverage-badge.samuelcolvin.workers.dev/redirect/bswck/mempip
	EOF
	)
	export DOCS_URL
	DOCS_URL=$(cat <<- 'EOF'
		https://mempip.readthedocs.io/en/latest/
	EOF
	)
	export PYPI_URL
	PYPI_URL=$(cat <<- 'EOF'
		https://pypi.org/project/mempip/
	EOF
	)
	export TIDELIFT_URL
	TIDELIFT_URL=$(cat <<- 'EOF'
		https://tidelift.com/subscription/pkg/pypi-mempip?utm_source=pypi-mempip
	EOF
	)
	export SKELETON
	SKELETON=$(cat <<- 'EOF'
	...
	SKELETON_REV=$(cat <<- 'EOF'
		https://github.com/bswck/skeleton/tree/0.0.2rc-210-g3b97536
	EOF
	)
	export SREV
	SREV=$(cat <<- 'EOF'
	...
	SKELETON_AND_REF=$(cat <<- 'EOF'
		bswck/skeleton@0.0.2rc-210-g3b97536
	EOF
	)
	export SNREF
	SNREF=$(cat <<- 'EOF'
	...
	GH_REPO_ARGS=$(cat <<- 'EOF'
		"bswck/mempip" --public --source=./ --remote=upstream --description="Install Python packages into volatile memory."
	EOF
	)
	export GH_ENSURE_ENV
	GH_ENSURE_ENV=$(cat <<- 'EOF'
		jq -n '{"deployment_branch_policy": {"protected_branches": false,"custom_branch_policies": true}}' | gh api -H "Accept: application/vnd.github+json" -X PUT "/repos/bswck/mempip/environments/$1" --silent --input -
	EOF
	)
